...
@section('title', 'Laporan Pusat - SIPERAH')

@section('content')
<div class="row align-items-center mb-4 no-print">
    <div class="col-md-6">
        <h1 class="h3 fw-bold mb-0">Laporan Pusat</h1>
        <p class="text-muted mb-0">Laporan rekapitulasi literase per POS untuk dikirim ke pusat</p>
    </div>
...
<div class="card p-0 overflow-hidden shadow-sm border-0" style="border-radius: 12px;">
    <div style="background: #f8fafc; padding: 20px; border-bottom: 2px solid var(--border); text-align: center;">
        <h2 class="fw-bold mb-1">REKAPITULASI SETORAN SUSU PER POS</h2>
        ...
        <table class="table mb-0 text-center">
            <thead style="background: #f1f5f9;">
                <tr>
                    ...
                    <th class="py-3 px-4" style="border: 1px solid #e2e8f0;">POS / KATEGORI</th>
                    <th class="py-3 px-4" style="border: 1px solid #e2e8f0;">VOLUME (L)</th>
                </tr>
            </thead>
            <tbody>
                @php 
                ...
                <tr>
                    ...
                    <td class="py-3 px-4" style="border: 1px solid #e2e8f0;">
                        <span class="badge bg-light text-dark fw-bold border">{{ strtoupper($data->pos) }}</span>
                    </td>
                    <td class="py-3 px-4 fw-bold text-primary" style="border: 1px solid #e2e8f0; font-size: 1rem; background: #fcfcfc;">{{ number_format($data->total, 1, ',', '.') }} L</td>
                </tr>
                @php 
                    $grandTotal += $data->total; 
                @endphp
                @empty
                <tr><td colspan="3" class="text-center py-5 text-muted">Tidak ada data untuk periode ini.</td></tr>
                @endforelse
            </tbody>
            <tfoot style="background: #f8fafc; border-top: 2px solid var(--border);">
                <tr class="fw-bold" style="page-break-inside: avoid;">
                    <td colspan="2" class="py-3 px-4 text-end">TOTAL KESELURUHAN</td>
                    <td class="py-3 px-4 fw-bold text-primary" style="font-size: 1.25rem; border: 2px solid var(--primary); text-align: right;">{{ number_format($grandTotal, 1, ',', '.') }} L</td>
                </tr>
            </tfoot>
        </table>
    </div>
